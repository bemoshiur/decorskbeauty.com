...
export const revalidate = 21600 // 6h (§13.2); also busted on variant publish

export async function GET() {
  const site = process.env.NEXT_PUBLIC_SITE_URL || 'https://decorskbeauty.com'
  const xml = itemsToRss(await buildFeedItems(site), { title: "Decor's K-Beauty — Meta feed", link: site })
